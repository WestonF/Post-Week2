...
//	for(int t = 0; t < t_end; t += timestep)
	{
		...
		for(int i = 0; i < N; i++)
		{
			...
			for(int j = 0; j < N; j++)
			{
				...
				//Force vector
				[&bodies, i, j, fval]() mutable throw()->void
				{
					double angle;
					...
					double force = fval();
					coordinate forcevec;
					bodies[i].totalforce.x += force * cos(angle);
					bodies[i].totalforce.y += force * sin(angle);
				};
			}
		}
